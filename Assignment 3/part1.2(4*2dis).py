...
def likelihoods(image, label, freq):
	...
	for i in range(NUM_TRAINING):
		...
		for line in range(7):
			for pixel in range(14):
				index = get_type(image, curr, 4*line, 2*pixel)
...
def get_type(image, curr, line, pixel):
	...
	temp[6] = image[curr+line+3][pixel]
	temp[7] = image[curr+line+3][pixel+1]
	record = []
	for i in range(8):
		if temp[i] == '+' or temp[i] == '#':
			record.append(i)
	...
	for n in range(NUM_TESTING):	#image loop, 1000
		...
		for i in range(NUM_DIGIT):	#digit loop, 10
			...
			for line in range(7):		#row loop, 28
				for pixel in range(14):	#column loop, 28
					index = get_type(image, curr, 4*line, 2*pixel)

...
#initiate a dict with size of 49 = 7^2
def init_likelihood():
	ret = {}
	for line in range(7):
...
def likelihoods(image, label, freq):
	...
	for i in range(NUM_TRAINING):
		...
		for line in range(7):
			for pixel in range(7):
				index = get_type(image, curr, 4*line, 4*pixel)
				ret[index][line,pixel][label[i]] += 1

	#Laplace Smoothing
	k = 0.1
	v = 65536		#feature can take 16 values
	for line in range(7):
		for pixel in range (7):
...
def get_type(image, curr, line, pixel):
	...
	temp[2] = image[curr+line][pixel+2]
	temp[3] = image[curr+line][pixel+3]
	temp[4] = image[curr+line+1][pixel]
	temp[5] = image[curr+line+1][pixel+1]
	temp[6] = image[curr+line+1][pixel+2]
	temp[7] = image[curr+line+1][pixel+3]
	temp[8] = image[curr+line+2][pixel]
	temp[9] = image[curr+line+2][pixel+1]
	temp[10] = image[curr+line+2][pixel+2]
	temp[11] = image[curr+line+2][pixel+3]
	temp[12] = image[curr+line+3][pixel]
	temp[13] = image[curr+line+3][pixel+1]
	temp[14] = image[curr+line+3][pixel+2]
	temp[15] = image[curr+line+3][pixel+3]
	record = []
	for i in range(16):
		if temp[i] == '+' or temp[i] == '#':
			record.append(i)
...
def MAP(likelihood, prior, image):
	...
	for n in range(NUM_TESTING):	#image loop, 1000
		...
		for i in range(NUM_DIGIT):	#digit loop, 10
			...
			for line in range(7):		#row loop, 28
				for pixel in range(7):	#column loop, 28
					index = get_type(image, curr, 4*line, 4*pixel)
